# train.py - Place in PSO-ToyBox/LLM/Benchmark/
...
def train_agent(
    ...
    try:
        ...
        # Log the error and exit or raise
        print(f"Error creating temporary environment: {e}")
        import traceback
        traceback.print_exc()
        # Decide how to handle this - exit, return, or raise
        # sys.exit(1) # Or return None, or raise e
        return # Returning None or similar might be better than sys.exit

    # Setup device
    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"\n--- Training Configuration (Vectorized Env) ---")
    print(f"Using device: {device}")
    print(f"Objective Functions: {len(objective_function_classes)}")
    print(f"Env Dim: {env_dim}, Particles: {env_particles}, Max PSO Steps: {env_max_steps}")
    ...
    print(f"Batch Size: {batch_size}, Start Steps: {start_steps}")
    print(f"Updates Per Step: {updates_per_step}")
    print(f"Save Freq Multiplier: {save_freq_multiplier}")
    print(f"---------------------------------------------\n")
    ...
    for func_index, selected_func_class in enumerate(objective_function_classes):
        ...
        for episode_num in range(episodes_per_function):
            ...
            while not terminated and not truncated:
                ...
                try:
                    ...
                    if np.isfinite(turn_final_gbest):
                     ...
                     print(f"Error during env.step() in episode {episode_num+1}, agent step {episode_agent_steps+1}: {e}")
                     import traceback
                     traceback.print_exc()
            ...
            print(f"  Final GBest: {final_gbest_for_log:.6e}")
        ...
        if results_log[func_name]:
             ...
             if finite_rewards:
                 print(f"  Avg Reward over {len(finite_rewards)} valid episodes: {np.mean(finite_rewards):.4f}")
             else:
                 print(f"  No finite reward values recorded for this function.")

             if func_gbests:
                  print(f"  Avg Final GBest over {len(func_gbests)} valid episodes: {np.mean(func_gbests):.6e}")
             else:
                  print(f"  No finite Final GBest values recorded for this function.")
        else:
             print(f"  No results logged for this function.")
        ...
        if (func_index + 1) % save_freq_multiplier == 0:
            try:
                ...
                print(f"Checkpoint saved after function {func_index + 1} ({func_name}) to {checkpoint_file}")
            except Exception as e:
    ...
    if all_final_rewards:
        ...
        print(f"Overall Mean Training Reward ({len(all_final_rewards)} valid Eps): {mean_reward:.4f} (μ) +/- {std_reward:.4f} (σ)")
        print(f"Overall Training Reward Variance: {var_reward:.6f}")
    else:
        print("No valid training reward data collected.")
    ...
    if all_final_gbests:
        ...
        print(f"Observed GBest Range: [{min_gbest:.6e}, {max_gbest:.6e}]")
        ...
        if gbest_range > 1e-12: # Check if range is significantly greater than zero
            ...
            print(f"Overall Mean NORMALIZED Final GBest ({len(all_final_gbests)} valid Eps): {mean_normalized_gbest:.6f} (μ) +/- {std_normalized_gbest:.6f} (σ)")
    ...
    if avg_rewards_per_func:
        ...
        if valid_func_names:
            ...
            print("No valid average rewards per function to plot.")
    else:
         print("No results logged for plotting average rewards per function.")
