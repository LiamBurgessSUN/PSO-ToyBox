...
try:
    ...
    class KnowledgeSharingStrategy:
        pass
...
class PSOVectorized:
    ...
    def __init__(self, objective_function, num_particles,
        ...
        if len(self.pbest_values) > 0 and np.isfinite(self.pbest_values[min_idx]):
            ...
            self.gbest_position = self.positions[0].copy() if self.num_particles > 0 else np.zeros(
                self.dim)  # Handle zero particles case
    ...
    def optimize_step(self, omega, c1, c2):
        ...
        try:
            ...
            if hasattr(self.objective_function, 'evaluate_matrix') and callable(
                ...
                if fitness_values.shape != (self.num_particles,):
                    log_warning(
                        f"evaluate_matrix returned unexpected shape {fitness_values.shape}. Expected ({self.num_particles},). Falling back to loop.",
                        module_name)
        ...
        value_to_add = self.gbest_value if np.isfinite(self.gbest_value) else self._gbest_history[
            -1] if self._gbest_history else float('inf')
