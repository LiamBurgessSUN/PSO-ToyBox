...
class PSOEnvVectorized(gym.Env):
    ...
    def step(self, action: np.ndarray):
        ...
        for i in range(self._current_nt):
            ...
            log_debug(f"  PSO Step {self.current_step}: gbest={current_gbest:.4e}, reward={step_reward:.4f}, converged={converged_this_step}", module_name)
    ...
    def _calculate_relative_reward(self, current_gbest: float) -> float:
        ...
        if not np.isclose(final_reward, reward):
             log_warning(f"Reward calculation resulted in value outside [0,1] ({reward:.4f}). Clipped to {final_reward:.4f}.", module_name)
        return final_reward
    ...
    def _get_obs(self, metrics: dict) -> np.ndarray:
         ...
         if not np.all(np.isfinite(obs_vec)):
             log_warning(f"Non-finite values detected in observation at step {self.current_step}. Replacing/Clipping. Original: {obs_vec}", module_name)
